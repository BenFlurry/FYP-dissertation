...
Medical appraisals are annual meetings with a supervisor aimed at reviewing progress against a personal development plan\cite{nhs_england_what_is_appraisal}. During this process, doctors review their practice, evaluate patient and colleague feedback, and define continuing professional development goals \cite{gmc_appraisal_guidance}. One way of supporting this process is to keep reflective notes on specific consultations. An AVT system can reduce some of the effort involved by storing reflection notes until an appraisal is required. This integration was requested by Dr. Atia Rafiq to support trainee GPs through that process.

\subsection{Project Ambience (LLMedic)}
The prequel to this project, developed over summer 2025 at UCL, hosted the Med42-70B model for medical summarisation on Intel GAUDI compute servers\cite{intel_gaudi3_2025}. This demonstrated that large medical language models could support clinical summarisation on Intel hardware, however it relied on a client-server architecture that was not appropriate for sensitive consultation data due to an insecure hosting environment. This project therefore shifts the focus from server-hosted inference to an offline, on-device solution \cite{intel_openvino_gaudi_aipc_2024}, while also extending the earlier summarisation work with AVT, appraisal support, and retrieval of medical guidance.

\section{Evaluation of Similar Solutions}
Evaluating existing solutions helps identify which problems are already well addressed and which constraints remain unresolved. By analysing the current solutions, this project can position itself to develop on the successes that other applications have achieved in this growing field. 
