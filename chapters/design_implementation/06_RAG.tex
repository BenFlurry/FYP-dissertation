...
\subsection{NICE API}
Our original approach was to download all NICE guidance PDF documents and bundle them with the application to be used for guidance retrieval. After applying for access to use the NICE API, we discussed our approach with a representative from their security team. He expressed the limitations that would be imposed on the project if the original plan of storing all medical guidance on device was undertaken. The NICE security team only allows access of their content through the API, and must guarantee that NICE information cannot be accessed outside of the UK. As a result, the only acceptable solution would require an API key to be hosted on either a private UCL or NHS network. The API solution was deemed out-of-scope for the proof-of-concept due to its incompatibility with public demonstrations and trials. After collaborating with my supervisors, we shifted our approach to the following design. 
...
The HNSW graph persists alongside the SQLite database in a \texttt{nice\_guidelines.bin} file which holds the calculated embeddings tree. Any PDF files uploaded by the user will be copied into the MSIX sandbox, then the SQLite database and embeddings tree will be updated

\subsection{Implementation}
\subsubsection{NPU Incompatability}
Although the design called for the MedCPT model to be ran on the NPU, there were implementation issues. After extensive debugging, a root cause for the model failing to load and crash the application could not be found. After consulting my external supervisor, using the NPU was de-prioritised due to a CPU implementation resulting in acceptable inference latency (TODO cite).
