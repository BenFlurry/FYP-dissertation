...
T6 & The development environment must be Microsoft Visual Studio with the MSIX packaging platform. \\
...
F9 & The user must be able to add and remove previously uploaded medical guidance documents. \\
...
Appraisal entries & Structured records containing a name, summary text, free-text notes, tags, and a creation timestamp stored in a JSON format. These must support search and filtering (F15, F16, F17). \\
...
F18 & Generic PDF support in RAG & MedCPT chunking and embedding pipeline applied to any uploaded PDF\\
\midrule
NF1 & Streaming transcription with $\leq$5\,s latency & Dedicated transcription threaded pipeline\\
...
NF5 & Med42-8B model loading within 60\,s & Early model loading on application launch in background thread \\
NF6 & Encryption at rest & Encrypted voice embedding file \\
NF7 & Zero outbound network traffic & No HTTP/socket calls in codebase\\
NF8 & Secure deletion & Data overwrite on delete operations \\
